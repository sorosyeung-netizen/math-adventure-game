...
const emojiCategories = {
    ...
    subtraction: ['⚽', '🏀', '🏐', '🎾', '🏓', '🏸', '⚾', '🏉'], // Sports balls only
    mixed: ['🍔', '🍟', '🍕', '🌭', '🥪', '🥐', '🍩', '🍪', '🍦', '🍭']     // Food only (replaced taco with candy)
};
...
function displayQuestion(num1, num2, operation, correctAnswer, wrongAnswer, type) {
    ...
    // Speak instruction based on operation type and mode
    setTimeout(() => {
        if (operation === '-' && type === 'subtraction') {
            speakSubtractionInstruction(num2, randomEmoji);
        } else if (type === 'mixed') {
            speakMixedInstruction(num1, num2, operation, randomEmoji);
        }
    }, 500); // Brief delay to let visual load
...
// Simplified subtraction take-away effect - animation only
function showSubtractionTakeAwayEffect() {
    ...
    // Create simple animation (voice is now handled in displayQuestion)
    container.innerHTML = '';
...
// Voice guidance for subtraction - 普通话版本
function speakSubtractionInstruction(takeAway, emoji) {
    if ('speechSynthesis' in window) {
        ...
        const emojiName = emojiNames[emoji] || '物品';
        const utterance = new SpeechSynthesisUtterance();
        utterance.text = `拿走${takeAway}个${emojiName}`;
...
// Voice guidance for mixed mode - 普通话版本
function speakMixedInstruction(num1, num2, operation, emoji) {
    if ('speechSynthesis' in window) {
        const emojiNames = {
            '🍔': '汉堡', '🍟': '薯条', '🍕': '披萨', '🌭': '热狗', 
            '🥪': '三明治', '🥐': '牛角面包', '🍩': 'donut', 
        ...
        const emojiName = emojiNames[emoji] || '食物';
        const utterance = new SpeechSynthesisUtterance();
        
        if (operation === '+') {
            utterance.text = `${num1}个${emojiName}加${num2}个${emojiName}等于多少`;
        } else {
            utterance.text = `${num1}个${emojiName}減${num2}个${emojiName}等于多少`;
        }
